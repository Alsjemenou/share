...
export default function UitnodigingPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const Kaart = ({ children }: { children: React.ReactNode }) => (
    <div className={`relative z-10 w-full max-w-sm ${merk?.kleur ? 'merk' : ''} ${achtergrondUrl ? 'self-start mr-auto' : ''}`} style={merk?.kleur ? ({ ['--brand']: merk.kleur } as React.CSSProperties) : undefined}>
      {heeftMerk ? (

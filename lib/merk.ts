...
export type Merk = {
  ...
  achtergrond_id: number | null // gebruiker-id om /api/merk/<id>/achtergrond op te bouwen
}
...
type Rij = { merk_naam: string | null; merk_subtitel: string | null; merk_kleur: string | null; merk_logo: string | null; merk_achtergrond: string | null; mag_branding: number }

const LEEG: Merk = { naam: null, subtitel: null, kleur: null, heeft_logo: false, logo_id: null, heeft_achtergrond: false, achtergrond_id: null }
...
export function haalMerk(gebruikerId: number): Merk {
  const db = getDb()
  const r = db.prepare('SELECT merk_naam, merk_subtitel, merk_kleur, merk_logo, merk_achtergrond, mag_branding FROM gebruiker WHERE id = ?')
  ...
  return {
    ...
    heeft_achtergrond: !!r.merk_achtergrond,
    achtergrond_id: r.merk_achtergrond ? gebruikerId : null,
  }
}
